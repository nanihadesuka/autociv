class BotManager {
  ...
  addBot(name, object) {
    ...
    if (!("react" in object)) object.react = function () {};

    if (!("load" in object)) object.load = function () {};
  ...
  react(msg) {
    ...
    for (let [name, bot] of this.list)
      if (bot.loaded && bot.active && bot.react(data)) return true;
    return false;
  }
    ...
    gamesetup: {
      ...
      types: {
        chat: function (msg) {
          ...
          if (
            Engine.GetPlayerGUID() === undefined ||
          ...
          return {
            type: "chat",
            receiver: splitRatingFromNick(
              g_PlayerAssignments[Engine.GetPlayerGUID()].name
            ).nick,
            sender: splitRatingFromNick(g_PlayerAssignments[msg.guid].name)
              .nick,
            message: msg.text,
          };
        },
  ...
  setMessageInterface(messageInterface) {
    ...
    if ("lobby" == this.messageInterface) {
      this.sendMessage = (text) => {
        if (Engine.HasXmppClient()) Engine.LobbySendMessage(text);
      };

      this.selfMessage = (text) => {
        let ftext = setStringTags(`== ${text}`, { font: "sans-bold-13" });
        g_LobbyHandler.lobbyPage.lobbyPage.panels.chatPanel.chatMessagesPanel.addText(
          Date.now() / 1000,
          ftext
        );
      };
      ...
      this.sendMessage = (text) => {
        if (Engine.HasNetClient()) Engine.SendNetworkChat(text);
      };

      this.selfMessage = (text) => {
        let ftext = setStringTags(`== ${text}`, { font: "sans-bold-13" });
        g_SetupWindow.pages.GameSetupPage.panels.chatPanel.chatMessagesPanel.addText(
          ftext
        );
      };
      ...
      g_Chat.ChatMessageHandler.registerMessageFormat(
        "autociv_self",
        new ChatMessageFormatAutocivSelf()
      );
    ...
    } else warn(`Invalid message interface ${this.messageInterface}`);
  }
}
...
function sendMessage(text) {
  ...
  for (let i = 0; i < numChunks; ++i)
    botManager.sendMessage(
      text.substr(i * sendMessage.maxChunkLength, sendMessage.maxChunkLength)
    );
}
...
botManager.addBot("playerReminder", {
  ...
  react: function (data) {
    ...
    if (!this.instance.hasValue(data.sender)) return;

    selfMessage(
      `playerReminder => ${data.sender} : ${this.instance.getValue(
        data.sender
      )}`
    );
  },
...
botManager.addBot("vote", {
  ...
  toggle: function (votingChoicesText) {
    if (votingChoicesText.trim() == "") {
      if (!this.active) {
        selfMessage(
          `You need to add choices. e.g  /vote diplo:teams:nomad:wonder`
        );
        return;
      }
      this.active = false;
    ...
    this.active = true;
    this.resetVoting();
    this.voteChoices = votingChoicesText
      .split(":")
      .slice(0, 7)
      .map((v) => v.trim());
    this.printVoting();
  ...
  printVoting: function () {
    ...
    for (let key in this.playerVote) choicesCount[this.playerVote[key]] += 1;
  ...
  react: function (data) {
    ...
    if (this.voteChoices[index] === undefined) return;

    let oldVote = this.playerVote[data.sender];
    if (oldVote === index) return;
...
botManager.addBot("autociv", {
  name: "AutoCiv bot",
  fuzzySearch: {
    /* Will be populated with a FuzzySet instance for each civ code on load*/
  },
  // Returns returns the civ code if civ name is found, otherwise undefined
  searchForCivInText: function (text) {
    // Lazy load
    if (!this.generateFuzzySearcher.loaded) this.generateFuzzySearcher();
    ...
    for (let id in this.fuzzySearch) {
      ...
      if (typeof res[0][1] != "string") continue;
  ...
  react: function (data) {
    ...
    if (
      firstWord == "play" &&
      Engine.ConfigDB_GetValue("user", "autociv.gamesetup.play.enabled") ==
        "true"
    ) {
  ...
  generateFuzzySearcher: function () {
    ...
    let customNamesCivs = Engine.ReadJSONFile(
      "autociv_data/civilizations.json"
    );
...
botManager.addBot("link", {
  ...
  parseURLs: function (text) {
    ...
    return text.match(
      /(sftp:\/\/|ftp:\/\/|http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?/gim
    );
  },
  findAndAdd: function (text) {
    let urls = this.parseURLs(text);
    if (urls != null) this.linkList.unshift(...urls.reverse());
  },
  openLink: function (text) {
    let i = parseInt(
      typeof text != "string" ? text : text.trim() != "" ? text : 0
    );
    ...
    if (!/(https?|ftp):.*/i.test(url)) url = "http://" + url.trim();
