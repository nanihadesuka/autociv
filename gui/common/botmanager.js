...
botManager.addBot("nub", {
    ...
    "react": function (data)
    {
        ...
        if (data.receiver != data.sender)
            sendMessage(`${data.sender} ${this.rand.random()}`);
    },
    "loadList": function ()
    {
        this.list = Engine.ReadJSONFile("autociv_data/nub.json");
        this.rand = new NonRepetitiveValListGet(this.list);
    }
});

botManager.addBot("noproblemo", {
    "name": "NoProblemo bot",
    "react": function (data)
    {
        if (data.type != "chat")
    ...
    "loadList": function ()
    {
        this.list = Engine.ReadJSONFile("autociv_data/noproblemo.json");
        this.rand = new NonRepetitiveValListGet(this.list);
    }
});

botManager.addBot("randomPhrase", {
    "name": "random phrases bot",
    "sendRandomMessage": function ()
    {
        // Lazy load
        if (this.list === undefined)
            this.loadList();

        sendMessage(this.rand.random());
    },
    "loadList": function ()
    {
        this.list = Engine.ReadJSONFile("autociv_data/randomPhrase.json");
        this.rand = new NonRepetitiveValListGet(this.list);
    }
});

botManager.addBot("didYouKnow", {
    "name": "random did-you-knows.com phrases",
    "sendRandomMessage": function ()
    {
        // Lazy load
        if (this.list === undefined)
    ...
    "loadList": function ()
    {
        this.list = Engine.ReadJSONFile("autociv_data/didYouKnow.json");
        this.rand = new NonRepetitiveValListGet(this.list);
    }
});

botManager.addBot("genPhrase", {
    "name": "nonsense generator",
    "sendRandomMessage": function ()
    {
        // Lazy load
        if (this.list === undefined)
        ...
        sendMessage(this.rand.genRandomText());
    },
    "loadList": function ()
    {
        this.list = Engine.ReadJSONFile("autociv_data/genPhrase.json");
        this.rand = new RandomChain(this.list);
    }
});
