...
g_OptionType["autociv_dropdown_runtime_load"] = {
  ...
  initGUI: (option, control) => {
    if (!option.list) option.list = global[option.autociv_list_load]();
...
displayOptions = function () {
  ...
  for (let i = 0; i < g_Options[g_TabCategorySelected].options.length; ++i) {
    ...
    let control = Engine.GetGUIObjectByName(
      "option_control_" +
        (g_OptionType[option.type].objectType ?? option.type) +
        "[" +
        i +
        "]"
    );
...
enableButtons = function () {
  g_Options[g_TabCategorySelected].options.forEach((option, i) => {
    ...
    const enabled =
      !option.dependencies || option.dependencies.every(isDependencyMet);
